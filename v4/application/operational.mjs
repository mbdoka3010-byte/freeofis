  ...
  const api = {
    ...
    async operationalData(route, query = {}) {
      ...
      if (route === 'purchases' || route === 'suppliers') {
        ...
        return { rows: scope(suppliers, businessId).map(supplier => { const purchases = receiptRows.filter(row => row.receipt.supplierId === supplier.id), supplierPayments = byNewest(scope(payments, businessId).filter(item => item.supplierId === supplier.id)); return { supplier, purchases, payments: supplierPayments, totalPurchasesMinor: purchases.reduce((n, row) => n + (row.totalMinor || 0), 0), totalPaidMinor: supplierPayments.reduce((n, item) => n + item.amountMinor, 0), outstandingMinor: purchases.reduce((n, row) => n + (row.outstandingMinor || 0), 0), lastPurchaseAt: purchases.map(row => row.receipt.transactionAt).sort().at(-1) || null, unpaidInvoices: purchases.filter(row => row.outstandingMinor > 0).map(row => ({ ...row.invoice, outstandingMinor: row.outstandingMinor })) }; }) };
      }
      if (route === 'reports') {
